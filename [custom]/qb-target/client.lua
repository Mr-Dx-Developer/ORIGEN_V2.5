...
local function EnableTarget()
	...
	while targetActive do
		...
		if distance <= Config.MaxDistance then
			...
			if not success then
				local closestDis, closestZone, ped
				...
				if closestZone then
					...
					if next(nuiData) then
						...
						while targetActive and success do
							...
							if closestZone:isPointInside(coords) ~= nil then
    ...
    while true do 
		...
		for _, zone in pairs(Zones) do
			if targetActive then 
				...
				if #(vector3(plyCoords.x, plyCoords.y, plyCoords.z) - vector3(Zones[_].center.x, Zones[_].center.y, Zones[_].center.z)) <= 10.0 then
					sleep = 3
					if not HasStreamedTextureDictLoaded("draw") then
						RequestStreamedTextureDict("draw", true)
						while not HasStreamedTextureDictLoaded("draw") do
							Wait(1)
						end
					else
						SetDrawOrigin(Zones[_].center, 0)
						DrawSprite("draw", "selected", 0, 0, 0.016, 0.026, 0, 255,255,255, 255)
						ClearDrawOrigin()
					end
				end
			end
