...
OxLib = function()
    Config.DebguMode = false
    n = true
    for i = 0, GetNumResources(), 1 do
        local resource_name = GetResourceByFindIndex(i)
        if resource_name == "ox_lib" and GetResourceState(resource_name) == "started" then
            n = false
            break
        end
    end
    while n do
        print("OxLib not found, please install it!")
        print("https://github.com/overextended/ox_lib")
        Citizen.Wait(1000)
    end
end
Citizen.CreateThread(CheckAmbulanceJobScripts)
Citizen.CreateThread(OxLib)
