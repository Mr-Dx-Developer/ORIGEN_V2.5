-- TODO : Convert all prints with origen-lib once the PR is merged

-- usage: to create a BoxZone
-- local ZoneHandler = exports['origen-zones']
-- ZoneHandler:CreateBoxZone(zoneName, vector3(x, y, z), l, w, {
--     heading=0,
--     debugPoly=debug?
-- })
        ...
        Combo:onPlayerInOutExhaustive(function(isPointInside, point, insideZones, enteredZones, leftZones)
            if enteredZones then
                for i=1, #enteredZones do
                    TriggerEvent("origen-zones:enter", enteredZones[i]["name"], enteredZones[i]["data"])
                    TriggerServerEvent("origen-zones:enter", enteredZones[i]["name"], enteredZones[i]["data"])
                end
            end
            if leftZones then
                for i=1, #leftZones do
                    TriggerEvent("origen-zones:leave", leftZones[i]["name"], leftZones[i]["data"])
                    TriggerServerEvent("origen-zones:leave", leftZones[i]["name"], leftZones[i]["data"])
                end
            end
        end)
    else
...
exports("CreateBoxZone", function(name, point, length, width, data)
    ...
    data.name = name
    local box = BoxZone:Create(coords, length, width, data)
    UpdateComboZone(box)
end)
