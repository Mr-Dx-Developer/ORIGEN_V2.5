...
RegisterNetEvent('jim-jobgarage:client:Garage:Menu', function(data)
	...
	if currentVeh.out and DoesEntityExist(currentVeh.current) then
		...
		vehicleMenu[#vehicleMenu+1] = { icon = "fas fa-car-burst", header = "Return Vehicle", params = { event = "jim-jobgarage:client:RemSpawn" } }
	else
...
RegisterNetEvent("jim-jobgarage:client:SpawnList", function(data)
	...
	if oldveh ~= 0 then
		...
		QBCore.Functions.SpawnVehicle(data.spawnName, function(veh)
			...
			--SetVehicleNumberPlateText(veh, string.sub(PlayerJob.label, 1, 5)..tostring(math.random(100, 999))) callsign = Player.metadata.callsign
