...
RegisterNUICallback("openProps", function(data, cb)
    flag = false
    TriggerCallback('pa-props:propItemControl', function(serverCb) 
        if serverCb then
...
RegisterNUICallback('saveBuild', function(data,cb)
    ...
    for k , v in pairs(PA.PropsAll) do
        if v.hash == tostring(objectHash) then
            ...
            TriggerServerEvent('pa-props:createProp', data, menuId)
        end
    end

end)

RegisterNetEvent('pa-props:setClient')
AddEventHandler('pa-props:setClient',function(data)
    cacheData = data
...
RegisterNetEvent('pa-props:notify')
AddEventHandler('pa-props:notify',function(data)
    Notify(data)
end)

RegisterNetEvent('pa-props:spawnProp')
AddEventHandler('pa-props:spawnProp',function(data,obj)
...
Citizen.CreateThread(function()
    while true do
         ...
         if NetworkIsPlayerActive(PlayerId()) then 
            playerData = GetPlayerData()
            TriggerServerEvent('pa-props:dataPostClient')
            Wait(5000)
            TriggerEvent('pa-props:createObject', cacheData)
            break
        end 
    end
end)
...
RegisterNetEvent('pa-props:createObject')
AddEventHandler('pa-props:createObject', function(cacheData)
...
RegisterNetEvent('pa-props:deleteProp')
AddEventHandler('pa-props:deleteProp',function()
    DeleteEntity(currentObject)
    DeleteObject(currentObject)
