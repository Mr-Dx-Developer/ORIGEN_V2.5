...
$(document).on('click', '.prop-item', function (e) {
    ...
    hash = $(this).data("hash");
    $.post(`http://pa-props/openProps`, JSON.stringify({itemName:itemName,propName:propName,hash:hash}), function (x) {
        if (x=='openProps') {
...
document.addEventListener('keydown', function(event) {
    if (event.key === 'Escape') {
        $("body").fadeOut(500);
        $.post(`http://pa-props/close`, JSON.stringify({ }), function (x) {});
    }
});
...
$(document).on('click', '.prop-settings', function (e) {
    ...
    if (dataType=="delete") {
        $.post(`http://pa-props/deleteProp`, JSON.stringify({ }), function (x) {});
    }
...
$(document).on('click', '.save-build-button', function (e) {
    ...
    $.post(`http://pa-props/saveBuild`, JSON.stringify({ }), function (x) {});
})
