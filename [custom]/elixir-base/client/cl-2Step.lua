...
RegisterNetEvent("2step:createsound")
AddEventHandler("2step:createsound", function(vehiclePos)
...
AddEventHandler("2step:Anti-lag", function()
        if antilag == false then
            antilag = true
			QBCore.Functions.Notify('Anti-Lag : Active', 'success')
        else
            antilag = false
			QBCore.Functions.Notify('Anti-Lag : DeActivated', 'error')
        end
end)
...
Citizen.CreateThread(function()
    while true do
		...
		if IsControlPressed(1, ConfigANTILAG.twoStepControl) then
			if IsPedInAnyVehicle(ped) then
				...
				if GetPedInVehicleSeat(pedVehicle, -1) == ped then
					...
					if RPM > 0.3 and RPM < 0.5 then
						for _,cars in pairs(ConfigANTILAG.TwoStepCars) do
							if GetHashKey(cars) == vehicleModel then
								...
								TriggerServerEvent("2step:triggersound", vehiclePos)
								activated = true
							  Wait(BackFireDelay)
							end
						end
		...
		if IsControlJustReleased(1, ConfigANTILAG.twoStepControl) then
