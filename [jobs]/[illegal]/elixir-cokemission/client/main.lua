    ...
    if Config.DisableBlackoutRequirement then
        exports['qb-target']:AddTargetModel({pedhash2}, {
            options = {
                {
                    type = "client",
                    event = "elixir-cokemission:client:start",
                    icon = "fas fa-user-secret",
                    label = "Give $10,000 and take the profession :)",
...
RegisterNetEvent('elixir-cokemission:client:start', function ()
	QBCore.Functions.TriggerCallback("elixir-cokemission:server:getCops", function(enoughCops)
    if enoughCops >= Config.MinimumPolice then
        QBCore.Functions.TriggerCallback("elixir-cokemission:server:coolc",function(isCooldown)
            if not isCooldown then
                QBCore.Functions.Progressbar("search_register", "Talking to..", 2200, false, true, {
                    ...
                    TriggerServerEvent('elixir-cokemission:server:startr')
                end, function() -- Cancel
                    QBCore.Functions.Notify("It is cancelled", 'error')
                end)
            else
                QBCore.Functions.Notify("Someone did this recently.", 'error')
            end
        end)
    else
        QBCore.Functions.Notify("I can't do this right now.", 'error')
	end
end)
end)
...
RegisterNetEvent('elixir-cokemission:client:targetexport', function()
...
exports['qb-target']:AddTargetModel('s_m_y_uscg_01', {
    options = {
        { 
            type = "client",
            event = "elixir-cokemission:SellItemCheck",
            icon = "fas fa-box",
            label = "Hand Package"
...
RegisterNetEvent('elixir-cokemission:SellItemCheck', function()
    QBCore.Functions.TriggerCallback("QBCore:HasItem", function(hasItem)
        if hasItem then
            SellCoke()
        else
            QBCore.Functions.Notify("You have no package", 'error')
        end
    end, Config.Vip)
end)

function SellCoke()
	QBCore.Functions.Progressbar("search_register", "Delivery of Packages", 5000, false, true, {
		disableMovement = true,
		disableCarMovement = true,
		disableMouse = false,
		disableCombat = true,
        ...
        TriggerServerEvent("elixir-cokemission:SellPackages")
...
RegisterCommand('boatspawntest', function()
    TriggerEvent('elixir-cokemission:client:boatactivate')
end)


RegisterNetEvent('elixir-cokemission:client:boatactivate', function()
...
RegisterNetEvent('elixir-cokemission:server:runactivate', function()
...
RegisterNetEvent('elixir-cokemission:client:information', function ()
    TriggerEvent('elixir-cokemission:ptfxparticle')
    TriggerEvent('elixir-cokemission:ptfxparticle2')
    TriggerEvent('elixir-cokemission:ptfxparticle3')
end)

-- Flare Creation

RegisterNetEvent("elixir-cokemission:ptfxparticle", function(method)
    local ptfx
...
RegisterNetEvent("elixir-cokemission:ptfxparticle2", function(method)
    local ptfx
...
RegisterNetEvent("elixir-cokemission:ptfxparticle3", function(method)
    local ptfx
...
RegisterNetEvent('elixir-cokemission:client:checkcoke')
AddEventHandler('elixir-cokemission:client:checkcoke', function()
    QBCore.Functions.TriggerCallback('QBCore:HasItem', function(hasItem)
        if hasItem then
            QBCore.Functions.Progressbar("search_register", "Destroying..", 3000, false, true, {
                disableMovement = true,
                disableCarMovement = true,
                disableMouse = false,
                disableCombat = true,
		...
		TriggerEvent('QBCore:Notify', src, "Successfully broken..", "success", 2500)
            end)
        else
...
RegisterNetEvent('elixir-cokemission:client:checkitem')
AddEventHandler('elixir-cokemission:client:checkitem', function()
    QBCore.Functions.TriggerCallback('QBCore:HasItem', function(hasItem)
        if hasItem then
            TriggerServerEvent('coke:processCoke')
        else
			QBCore.Functions.Notify("You have no seal package.", 'error')
        end
    end, "sealedpackage")
end)

RegisterNetEvent('elixir-cokemission:now-can-collect-package', function()
    QBCore.Functions.Notify("You can now collect your belongings.", 'success')
	exports['qb-target']:AddTargetModel({pedhash}, {
		options = {
			{
				event = "collect-coke",
				icon = "fas fa-box",
				label = "Collect the items",
			}
		},
		distance = 2.0
	})
...
RegisterNetEvent('collect-coke', function()
	...
	exports['qb-target']:AddTargetModel({pedhash}, {
		options = {
			{
				event = "elixir-cokemission:client:checkitem",
				icon = "fas fa-box",
	...
	TriggerServerEvent('elixir-cokemission:server:packagereward')

end)


RegisterNetEvent('elixir-cokemission:client:removetarget', function()
	exports['qb-target']:RemoveTargetModel({pedhash}, {
		'Collect the items'
	})
end)

RegisterNetEvent('elixir-cokemission:client:removetarget2', function()    
    exports['qb-target']:RemoveTargetModel({pedhash}, {
        'Process Packages'
    })
end)

RegisterNetEvent('elixir-cokemission:client:removetarget3', function()    
...
RegisterNetEvent('elixir-cokemission:client:end', function()
    ...
    TriggerEvent('elixir-cokemission:client:endRun')
end)


RegisterNetEvent('elixir-cokemission:client:endRun', function()
...
RegisterNetEvent('sd-blackout:client:interactionsync', function()
    TriggerEvent('elixir-cokemission:client:addtarget', -1)
end)

RegisterNetEvent('elixir-cokemission:client:addtarget', function()
        ...
        options = {
            {
                ...
                event = "elixir-cokemission:client:start",
                icon = "fas fa-user-secret",
                label = "Pay me 100,000 dollars for me :)",
...
--                 end)
--             else
--                 QBCore.Functions.Notify("Geçenlerde biri bunu yaptı.", 'error')
--             end
--         end)
--     else
--         QBCore.Functions.Notify("Bunu şu anda yapamam.", 'error')
-- 	end
-- end)
-- end)
...
--             Citizen.Wait(1000)
--         else
--             QBCore.Functions.Notify("Sende C4 yok!", 'error')
--         end
--     end, "weapon_stickybomb")
-- end)
...
--      exports['qb-target']:RemoveCircleZone(651.99, 101.11, 81.16, {
-- 		"Plant Explosive"
-- 	})
...
function bombanime()
    ...
    TriggerEvent('inventory:client:ItemBox', QBCore.Shared.Items["weapon_stickybomb"], "remove")
    TriggerServerEvent("QBCore:Server:RemoveItem", "weapon_stickybomb", 1) 
    QBCore.Functions.Notify("The explosive was placed!To escape!", 'success')
...
RegisterNetEvent('elixir-cokemission:client:signoff', function()

	exports['qb-target']:AddTargetModel({pedhash2}, {
		options = {
			{
				type = "client",
				event = "elixir-cokemission:client:end",
				icon = "fas fa-circle",
				label = "Signout Job",
...
RegisterNetEvent('elixir-cokemission:client:email', function() 
	if Config.SendEmail then
        QBCore.Functions.Notify("I will send an e-mail immediately!", 'success')
    CokeStart()
	end
end)
...
function CokeStart()
	...
	message = ('A friend of mine was able to refill the hidden underwater warehouses on the west coast of LS, and they were marked with a flare.To reach there, you can play a boat from the Alamo Sea or use your own boat.I marked all the necessary places in your GPS.Bring those packages back to me.Good luck!'),
	})
	Citizen.Wait(3000)
end
-- ElixirFW
