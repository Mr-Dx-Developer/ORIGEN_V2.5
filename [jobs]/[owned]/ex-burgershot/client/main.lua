...
AddEventHandler("rz-burgershot:tray3", function()
    TriggerEvent("inventory:client:SetCurrentStash", "tray3")
    TriggerServerEvent("inventory:server:OpenInventory", "stash", "tray3", {
        maxweight = 5000,
        slots = 5,
    })
end)
...
AddEventHandler("rz-burgershot:tray4", function()
    TriggerEvent("inventory:client:SetCurrentStash", "tray4")
    TriggerServerEvent("inventory:server:OpenInventory", "stash", "tray4", {
        maxweight = 5000,
        slots = 5,
    })
end)
...
RegisterNetEvent("ex-burgershot:bill", function()
    ...
    if bill ~= nil then
        ...
        TriggerServerEvent("ex-burgershot:server:billPlayer", bill.citizenid, bill.billprice)
    end
end)